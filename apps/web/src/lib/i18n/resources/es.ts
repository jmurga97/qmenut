export default {
  allergens: {
    gluten: "Gluten",
    crustaceans: "Crustáceos",
    eggs: "Huevos",
    fish: "Pescado",
    peanuts: "Cacahuetes",
    soybeans: "Soja",
    milk: "Leche",
    nuts: "Frutos de cáscara",
    celery: "Apio",
    mustard: "Mostaza",
    sesame: "Sésamo",
    sulphites: "Sulfitos",
    lupin: "Altramuces",
    molluscs: "Moluscos",
  },
  common: {
    currencyLabel: "Moneda",
    languageLabel: "Idioma",
    primaryNavigationLabel: "Navegación principal",
    navigation: {
      menu: "Inicio",
      destacados: "Destacados",
      contact: "Contacto",
      loyalty: "Puntos",
    },
  },
  menu: {
    categoryNavigationLabel: "Categorías del menú",
    closeLabel: "Cerrar",
    dishCount_one: "{{count}} plato",
    dishCount_many: "{{count}} platos",
    dishCount_other: "{{count}} platos",
    emptyLabel: "No hay platos disponibles",
    extrasLabel: "Extras",
    allergensLabel: "Alérgenos",
    heroLabel: "Menú del día",
    photoLabel: "Foto del plato",
    comboAvailable: "Disponible en combo",
    comboPriceLabel: "Precio total del combo",
    recommended: "Recomendado",
    featuredBadges: {
      fine: "Recomendación del chef",
      her: "Especial de la casa",
      cafe: "Nuestro favorito",
      fast: "Nuestro favorito",
      tapas: "El plato estrella",
    },
  },
  destacados: {
    seoTitle: "Destacados – {{name}}",
    seoDescription: "Platos recomendados y ofertas actuales de {{name}}",
    page: {
      title: "Destacados",
      subtitle: "Nuestras recomendaciones y ofertas actuales",
      recommendedTitle: "Recomendados",
      recommendedEmptyLabel: "Todavía no hay platos recomendados",
      empty: {
        title: "Todavía no hay nada por aquí",
        description: "Estamos preparando nuevas recomendaciones y ofertas. Vuelve pronto.",
      },
    },
  },
  promos: {
    page: {
      title: "Promociones",
      emptyLabel: "No hay promociones disponibles",
      availableToday: "Disponible hoy",
    },
    badges: {
      happyHour: "happy",
      menu: "menú",
      offer: "oferta",
    },
    types: {
      percentage_discount: "Descuento aplicado durante la vigencia indicada.",
      special_price: "Precio especial durante la vigencia indicada.",
      daily_menu: "Menú disponible durante la vigencia indicada.",
      happy_hour: "Oferta por tiempo limitado.",
      two_for_one: "Llévate dos unidades y paga una.",
    },
  },
  contact: {
    seoTitle: "Contacto – {{name}}",
    seoDescription: "Dirección, teléfono y contacto de {{name}}{{address}}",
    page: {
      title: "Contacto",
      subtitle: "Dónde encontrarnos y cómo hablar con nosotros",
      mapLabel: "Ubicación de {{name}}",
      locationSectionLabel_one: "Ubicación",
      locationSectionLabel_many: "Ubicaciones",
      locationSectionLabel_other: "Ubicaciones",
      sitesSectionLabel: "Sedes",
      socialLinksLabel: "Redes sociales",
      mapAriaLabel_one: "Mapa con una sucursal",
      mapAriaLabel_many: "Mapa con {{count}} sucursales",
      mapAriaLabel_other: "Mapa con {{count}} sucursales",
      openGoogleMapsLabel: "Abrir en Google Maps",
      viewMenuLabel: "Ver carta",
      scheduleUnavailable: "Consulta el horario",
      callLabel: "Llamar",
      actionsLabel: "Acciones de contacto",
      whatsappLabel: "WhatsApp",
      mapLinkLabel: "Cómo llegar",
    },
    reviews: {
      allReviews: "Ver todas las reseñas",
      beFirst: "Sé la primera persona en opinar",
      googleRating: "Valoración en Google",
      googleReviewer: "Usuario de Google Maps",
      heading: "Lo que cuentan nuestros clientes",
      leaveReview: "Escribir una reseña en Google",
      loading: "Cargando reseñas de Google",
      ratingCount_one: "{{count}} reseña",
      ratingCount_many: "{{count}} reseñas",
      ratingCount_other: "{{count}} reseñas",
      ratingOutOfFive: "{{rating}} de 5 estrellas",
      readOnGoogle: "Leer en Google Maps",
      relevanceNotice: "Reseñas seleccionadas por Google y ordenadas por relevancia.",
      translated: "Traducida por Google · Ver original",
      visited: "Visita: {{date}}",
    },
  },
  legal: {
    navLabel: "Páginas legales",
    legalNoticeLink: "Aviso legal",
    privacyLink: "Política de privacidad",
    privacy: {
      title: "Política de privacidad",
      subtitle: "Protección de datos",
      seoTitle: "Política de privacidad – {{name}}",
      seoDescription: "Política de privacidad y protección de datos de {{name}}",
    },
    legalNotice: {
      title: "Aviso legal",
      subtitle: "Información del titular",
      seoTitle: "Aviso legal – {{name}}",
      seoDescription: "Aviso legal e información del titular de {{name}}",
    },
  },
  install: {
    title: "Lleva la carta en tu móvil",
    body: "Añádela a tu pantalla de inicio y ábrela de un toque, incluso sin conexión.",
    action: "Instalar",
    dismissLabel: "Cerrar",
    ios: {
      title: "Cómo añadirla en iPhone",
      step1: "Abre el botón Compartir o el menú de tu navegador",
      step2: "Elige «Añadir a pantalla de inicio»",
      step3: "Si no aparece la opción, abre esta página en Safari",
    },
  },
  offline: {
    title: "Sin conexión",
    body: "No hemos podido cargar esta página. Las páginas que ya has visitado siguen disponibles.",
    retry: "Reintentar",
    seoTitle: "Sin conexión – {{name}}",
  },
  loyalty: {
    seoTitle: "Fidelización – {{name}}",
    seoDescription: "Tarjeta de fidelización y premios de {{name}}",
    page: {
      title: "Fidelización",
      subtitle: "Tus visitas se convierten en premios",
    },
    loading: "Cargando tu tarjeta de fidelización",
    unavailable: {
      title: "Todavía no hay premios disponibles",
      body: "El establecimiento publicará aquí su programa de fidelización cuando esté listo.",
    },
    intro: {
      title: "Visítanos y gana puntos",
      body_one: "Cada visita suma un sello. Con {{count}} sello te llevas {{reward}}.",
      body_many: "Cada visita suma un sello. Con {{count}} sellos te llevas {{reward}}.",
      body_other: "Cada visita suma un sello. Con {{count}} sellos te llevas {{reward}}.",
      hint: "Escanea el QR de la mesa en el local para activar tu tarjeta y canjear premios.",
      menuLink: "Ver la carta",
    },
    signup: {
      pitch_one: "Cada visita, un sello. {{count}} sello, {{reward}}.",
      pitch_many: "Cada visita, un sello. {{count}} sellos, {{reward}}.",
      pitch_other: "Cada visita, un sello. {{count}} sellos, {{reward}}.",
      explainer: "Crea tu tarjeta con tu email y recupérala desde cualquier dispositivo.",
      emailLabel: "Email",
      emailPlaceholder: "tu@email.com",
      consentLabel: "Acepto el tratamiento de mi email para gestionar mi tarjeta de fidelización.",
      consentPrivacyLink: "Leer la política de privacidad",
      consentRequired: "Debes aceptar la política de privacidad para crear la tarjeta.",
      submit: "Crear mi tarjeta →",
      creating: "Creando…",
      reassurance: "Sin contraseña · solo tu email · válida en todas las sedes",
    },
    consent: {
      pitch: "Nueva política de privacidad",
      explainer: "Confirma tu email ({{email}}) para desbloquear tu tarjeta. Tus sellos y premios están a salvo.",
      emailLabel: "Email",
      emailPlaceholder: "tu@email.com",
      consentLabel: "Acepto el tratamiento de mi email para gestionar mi tarjeta de fidelización.",
      consentPrivacyLink: "Leer la política de privacidad",
      consentRequired: "Debes aceptar la política de privacidad para desbloquear la tarjeta.",
      submit: "Confirmar y desbloquear →",
      confirming: "Confirmando…",
      reassurance: "Sin contraseña · solo tu email",
    },
    card: {
      progress: "Tus sellos",
      gridLabel_one: "{{filled}} de {{total}} sello",
      gridLabel_many: "{{filled}} de {{total}} sellos",
      gridLabel_other: "{{filled}} de {{total}} sellos",
      requestStamp: "Pedir mi sello",
      redeemed: "Premio canjeado",
      redeemedFooter: "saldo {{balance}}/{{total}} · tu próxima visita ya suma",
    },
    code: {
      networkError: "No se pudo conectar. Comprueba tu conexión y vuelve a introducir el código.",
      title: "Introduce el código",
      hint: "Pídele el código al personal",
      inputLabel: "Código de sala de cuatro dígitos",
      footnote: "el código cambia cada pocos minutos · 1 sello por visita",
      invalid: "Código caducado o erróneo · reintenta",
      throttled: "Espera 30 s antes de reintentar",
      already: "Ya tienes el sello de esta visita",
    },
    reward: {
      stamps_one: "sello",
      stamps_many: "sellos",
      stamps_other: "sellos",
      remaining_one: "falta {{count}}",
      remaining_many: "faltan {{count}}",
      remaining_other: "faltan {{count}}",
      redeem: "Canjear",
      redeeming: "Enviando…",
      locked: "Escanea el QR del local para canjear",
    },
    redeem: {
      badge: "Canje · {{reward}}",
      pendingTitle: "Esperando confirmación del personal…",
      pendingHint: "Enséñale esta pantalla a quien te atiende",
      countdown: "caduca en {{time}}",
      cancel: "Cancelar",
      rejectedTitle: "El personal no validó el canje",
      expiredTitle: "Pasaron 10 min sin validar",
      retry: "Reintentar",
      return: "Volver a canjear",
    },
    errors: {
      acceptConsent: "No pudimos confirmar tu tarjeta. Revisa el email e inténtalo de nuevo.",
      createCard: "No pudimos crear tu tarjeta. Revisa el email e inténtalo de nuevo.",
      loadCard: "No pudimos cargar tu tarjeta. Inténtalo de nuevo.",
    },
  },
} as const;
