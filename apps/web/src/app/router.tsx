...
export function getRouter() {
  ...
  const router = createTanStackRouter({
    ...
    // Router matches use useSyncExternalStore: the router owns navigation snapshots.
    // Do not wrap the same route swap in React ViewTransition.
    defaultViewTransition: {
      // TanStack intentionally uses false to disable a transition and string[] to enable one.
      // eslint-disable-next-line sonarjs/function-return-type
      types: ({ fromLocation, toLocation }) => {
        if (!fromLocation || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return false;
