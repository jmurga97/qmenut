...
import { buildMenuUrl, buildQrFileBase, downloadQr, renderQrPreview } from "../services";

const qrFormSchema = z.object({ size: z.enum(["512", "1024", "2048"]) });
type QrFormValues = z.infer<typeof qrFormSchema>;
export function useQrController(host: string) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [error, setError] = useState<unknown>(null);
  const [copied, setCopied] = useState(false);
  const form = useForm<QrFormValues>({ resolver: zodResolver(qrFormSchema), defaultValues: { size: "1024" } });
  const size = Number(useWatch({ control: form.control, name: "size" }));
  const url = buildMenuUrl(host);
  const fileBase = buildQrFileBase(host);
  useEffect(() => {
