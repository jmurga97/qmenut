...
Copia `apps/api/.dev.vars.example` a `apps/api/.dev.vars` y
`apps/tenant-config/.dev.vars.example` a `apps/tenant-config/.dev.vars`; rellena
los secretos y conserva el mismo `THEME_WORKER_TOKEN` en ambos workers.
...
- `admin.billing.checkout` → crea/recupera el customer del restaurante
  ...
  la URL; la SPA redirige. Customers y sesiones usan claves de idempotencia
  deterministas para evitar duplicados bajo peticiones concurrentes. La clave de
  checkout se conserva por sucursal durante la ventana de idempotencia de Stripe
  (24 h): un nuevo checkout con otro plan dentro de esa ventana devuelve conflicto;
  los cambios de plan deben hacerse desde Customer Portal.
...
- Los estados de Stripe se mapean sobre el enum de 4 valores de
  `branch_subscriptions` (`map-stripe-status.ts`). `active`, `trialing` y
  `past_due` conservan el plan contratado (gracia); `canceled` degrada el cache
  `branches.plan_code` a `basic`. Para aplicar límites en endpoints, la costura
  `getBranchEntitlement` + `requirePlan` deriva siempre el plan efectivo desde
  `branch_subscriptions`; queda pendiente conectarla a funciones concretas cuando
  exista el catálogo de diferencias entre planes.
