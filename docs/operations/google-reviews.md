...
2. Create a dedicated API key. Do not reuse `GOOGLE_MAPS_API_KEY`, which is for branch
   geocoding.
...
6. In Google Maps Platform quotas, set daily request limits for Text Search (New) and Place
   Details (New). Start from expected traffic plus a deliberate safety margin and revisit the
   values after observing production usage. Place Details requests that include reviews use
   the Enterprise + Atmosphere billing tier.
