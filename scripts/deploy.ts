...
function parseArguments(arguments_: string[]): { environment: Environment; migrationFlags: string[] } {
...
try {
  await step(`Preflight ${environment}`, () =>
    run(["bun", "apps/api/scripts/check-deployment-config.ts", environment], rootDirectory),
  );
  ...
  await step(`D1 migrations ${environment}`, async () => {
    ...
    await run(["bun", "apps/api/scripts/apply-db-migrations.ts", environment, ...flags], rootDirectory);
  });
  ...
  await step("Deploy web", async () => {
    ...
    await run(["bunx", "wrangler", "deploy"], appDirectory("web"), { CLOUDFLARE_ENV: environment });
  });
